...
import '../../index.css'
import { Drawer, InputBase, List, ListItem,Hidden, Box } from '@material-ui/core';
...
const handleShowSearch = (open) => event => {
    ...
    <>
       ...
       <span><button className="delFromCart" style={{marginLeft:10+'px', paddingTop : 5+'px',paddingBottom:5+'px',marginBottom:5+'px'}} onClick={handleCart(false)}>Close</button> </span> 
        ...
        <Toolbar>
           ...
           <div className="searchWrap">
            ...
            <Box display={{ xs:'block',sm: 'block',md:'block',lg:'none'}}>
                ...
                { isShowSearch ?  <span className="hoverCursor searchIconRes" onClick={handleShowSearch(false)} >
                <CloseIcon style={{ fontSize : 30+'px' }}/>
                </span> : <span className="hoverCursor searchIconRes" onClick={handleShowSearch(true)} >
          ...
          <ShoppingCartIcon className="navbarIcon"  style={{ fontSize : 30+'px' }}/>
          { cart.cart.length > 0 &&
        ...
        <SearchIcon style={{color : 'black' , marginLeft : 15+'px'}}></SearchIcon>
        <InputBase style={{ marginLeft : 20+'px' ,paddingRight : 20+'px',width : 90+'%' ,borderColor : 'black' }} placeholder="Search…" variant="outlined"  
        type="text" id="searchText" name="searchText" onChange={onChange} />
        </div>
        </Hidden>}
